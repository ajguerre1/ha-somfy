...
| OPEN-01 | **Entity-ID reconciliation is unresolved.** Renaming the Homebridge entities to `*_2` frees IDs like `cover.roomb_shades`, but this integration would still create `cover.roomb_sh`, because entity IDs derive from the gateway's group names (`RoomB SH`) and those do not match the HA names (`RoomB Shades`). Freeing the old ID alone does not make the new entity adopt it. Needs a decision before entities are created: name the group entities to match the HA names, rename groups on the gateway, or accept the new IDs and update dashboards. | H | open | Naming scheme agreed and applied before the config entry is added |
