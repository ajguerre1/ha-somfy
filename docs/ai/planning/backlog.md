...
| MIGRATE-05 | **`077537` "RoomB B/O 3" is in no group.** `RoomB B/O` has 2 members but three RoomB blackout motors exist, so `cover.roomb_b_o` will move 2 of 3. Also the node that missed a discovery pass — possibly only partly commissioned. Add it to the group in the UAI+ web UI, or drive it via its individual motor entity. | M | open | The third RoomB blackout motor is reachable |
