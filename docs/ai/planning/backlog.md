...
| HW-04 | **Cross-check names against group membership at discovery.** A node whose name disagrees with its group's name is worth a warning: that mismatch was the only signal that caught HW-03, and it was ignored. Cheap to add, since both are already fetched per node. Note this is now *partly* superseded by NAME-01 — a first-read placeholder is precisely a name that matches its group — so the warning should fire on the settled name, not the first. | L | open | Discovery logs a warning on name/group mismatch |
...
| WORK-05 | **The HACS store-list icon will resolve after download + restart. No repo change needed.** Chain traced through source: HACS `hacs-dashboard.ts:330` calls the HA frontend's `brandsUrl()`, which since HA 2026.3 returns `/api/brands/integration/{domain}/{image}?token=…` and **never** hits the CDN directly. HA core `brands/__init__.py:257` tries custom-integration local files **first**, then the CDN. `_serve_from_custom_integration` requires the domain to be in `async_get_custom_components()` — i.e. **downloaded and HA restarted**; a config entry is *not* required. `has_branding` is simply `"brand" in top_level_files` (`loader.py:894`), which our folder satisfies. HACS requests `dark_icon.png` in dark mode, but `_read_brand_file` walks `IMAGE_FALLBACKS`, and `dark_icon.png → icon.png`, so no dark variant is needed. Pre-install the lookup finds nothing local and falls through to the CDN placeholder — the reported symptom, and expected. | L | open | Icon renders in the HACS store list after download + restart |
