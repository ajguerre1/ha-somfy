...
| **PROTO-07** | **Closed, not worth pursuing.** `somfy_devices.json` returns 403 with HTTP basic auth as well as without, as does `somfy_presets.json`. The singular `somfy_device.json?<node>` needs no auth and returns strictly more per node. | 2026-07-29 | `scripts/probe_open_questions.py` |
